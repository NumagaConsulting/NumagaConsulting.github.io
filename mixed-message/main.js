  ...
  quoteByGenreOrAuthor(quoteGenre, quoteAuthor) {
    ...
    if (quoteGenre && quoteAuthor) {
      const quotes = obj.filter((element) => {
        return (
          element.GENRE.trim().toLowerCase() ===
            quoteGenre.trim().toLowerCase() &&
          element.AUTHOR.trim().toLowerCase() ===
            quoteAuthor.trim().toLowerCase()
        );
      });
      if (quotes.length) {
        return quotes;
    ...
    if (quoteGenre) {
      ...
      if (quoteByGenre.length) {
        console.log(
          `Can't find any quote by ${quoteAuthor}. We will randomly pick one for you.`
        );
        return quoteByGenre;
      }
      return 'not found';
